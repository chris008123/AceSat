...
def demo_student(student_id) -> Student:
    """Mirrors ai-data's own `demo_student` fixture and the demo account in
    Deployment_architecture.txt §12.
    """
    return Student(
...
def declining_reading_inference_responses(student_id):
    """Same shape as ai-data's own fixture of the same name: strong-ish
    early on, then a recent slump — enough evidence to trigger a
    DECLINING trend and a below-mastery weak topic.
    """
    responses = []
...
@pytest.fixture
def mixed_responses(student_id, declining_reading_inference_responses, strong_algebra_responses):
    return declining_reading_inference_responses + strong_algebra_responses
